...
import io.github.bedwarsrel.events.BedwarsFriendlyBreakEvent;
...
public class BlockListener extends BaseListener {
	...
	public void onBlockGrow(BlockGrowEvent grow) {
		...
		Game game = BedwarsRel.getInstance().getGameManager().getGameByLocation(grow.getBlock().getLocation());
		if (game == null) {
			return;
		}
			...
			if (!BedwarsRel.getInstance().getBooleanConfig("friendlybreak", true)) {
				...
				for (Player player : playerTeam.getPlayers()) {
					...
					if (player.getLocation().getBlock().getRelative(BlockFace.DOWN).equals(e.getBlock())) {
						...
						BedwarsFriendlyBreakEvent call = new BedwarsFriendlyBreakEvent(g, p, playerTeam);
						BedwarsRel.getInstance().getServer().getPluginManager().callEvent(call);
						return;
					}
				}
			}
	...
	public void onForm(BlockFormEvent form) {
		...
		Game game = BedwarsRel.getInstance().getGameManager().getGameByLocation(form.getBlock().getLocation());
		if (game == null) {
			return;
		}
	...
	public void onIgnite(BlockIgniteEvent ignite) {
		...
		if (ignite.getIgnitingBlock() == null) {
			if (ignite.getIgnitingEntity() instanceof Player) {
				game = BedwarsRel.getInstance().getGameManager().getGameOfPlayer((Player) ignite.getIgnitingEntity());
			} else {
				game = BedwarsRel.getInstance().getGameManager()
						.getGameByLocation(ignite.getIgnitingEntity().getLocation());
			}
		} else {
			game = BedwarsRel.getInstance().getGameManager().getGameByLocation(ignite.getIgnitingBlock().getLocation());
		}
	...
	public void onPlace(BlockPlaceEvent bpe) {
		...
		if (game.getState() == GameState.RUNNING) {
			...
			if (replacedBlock != null && !BedwarsRel.getInstance().getBooleanConfig("place-in-liquid", true)
	...
	public void onSpread(BlockSpreadEvent spread) {
		...
		Game game = BedwarsRel.getInstance().getGameManager().getGameByLocation(spread.getBlock().getLocation());
		if (game == null) {
			return;
		}
